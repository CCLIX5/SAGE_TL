...
%   disp(mean(dist2_x_de,2))
  dist2_x_nu=repmat(x_ce2',[1 n_nu])+repmat(x_nu2,[b 1])-2*x_ce'*x_nu;
%   disp(mean(dist2_x_nu,2))
%   disp(size(x_ce2'))
%   disp(size(repmat(x_ce2',[1 n_de])))
...
%     disp(lambda_chosen_index)
  ...
  disp(mean(score_cv,'all')) 
  disp(mean(score_cv_tmp,'all')) 
...
%   disp(mean(alphat,'all'))
  alphah=max(0,alphat);
%   disp(alphah)
  wh_x_de=alphah'*K_de;
  disp(mean(wh_x_de,'all'))
%   testt= K_de*K_de'/n_de+lambda_chosen*eye(b);
%   S = svd(testt);
%   disp(S)
